...
def aggregate_pipeline_stats(
    ...
    aggregate_length_filter_stats(), filter_groups(), and make_tables() —
    all of which live under fixed subdirectory names of `listdir` — and
    produces a single JSON-serialisable summary combining funnel counts
    (how many groups survive each stage) with a breakdown of *why* groups
    or sequences were dropped at each stage.
    ...
    Args:
        listdir:     Root directory for this list (e.g. outdir/list1, or
                     outdir itself in single-list mode). Expected to contain
                     'length_filter/', 'length_filter_stats/',
                     'group_filter/', 'group_filter_stats/', and 'tables/'
                     as produced by _run_single_list().
